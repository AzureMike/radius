...
This resolves the latest `resource-types-contrib` `main` commit, pins it as a commit SHA in `deploy/manifest/defaults.yaml` (`source.ref`), and copies the manifest files listed there into `deploy/manifest/built-in-providers/`. To pin a specific release tag or commit instead of the latest `main`, pass `RESOURCE_TYPES_REF`, for example `make update-resource-types RESOURCE_TYPES_REF=v0.56.0`. Review the diff to confirm the changes are expected.
...
2. **Pin to last known good version**: Revert the `source.ref` change in `deploy/manifest/defaults.yaml` to keep the previous `resource-types-contrib` revision and run `make sync-resource-types` to restore the matching manifests.
