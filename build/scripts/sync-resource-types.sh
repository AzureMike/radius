...
# the same (repo, ref) are fetched once. The selected manifest files are copied
# into every destination directory and stale managed files are pruned.
#
...
copy_manifests() {
    local tmp_root pairs_file i=0 repo ref dir entry ns rel type src dest
    tmp_root="$(mktemp -d)"
...
# (manual core manifests are always preserved).
prune_stale() {
