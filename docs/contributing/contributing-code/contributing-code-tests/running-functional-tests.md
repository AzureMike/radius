...
THUMBPRINT="$(echo | openssl s_client -servername "$ISSUER_HOST" -connect "${ISSUER_HOST}:443" 2>/dev/null \
...
- **LRT AWS tests fail with `AccessDenied` on `AssumeRoleWithWebIdentity`.** The LRT cluster's OIDC issuer is not trusted by `FUNC_TEST_RAD_IRSA_ROLE`, or the service-account subjects don't match. Re-run the trust setup in [Point the AWS IRSA role at the LRT cluster](#point-the-aws-irsa-role-at-the-lrt-cluster) and confirm the AKS issuer is registered as an IAM OIDC provider.
